...
from tracker.an_api_wrapper import *
import re
...
def create_one_on_one(request):
  if request.method == 'POST':
    ...
    if(errors == []):
      ...
      an_response = an_post(current_user.first_name, current_user.last_name, current_user.email, contact_first_name, contact_last_name, contact_email, oneonone_notes)
      ...
      if(an_response[0] == 200):
        ...
        new_oneonone = OneOnOne(user=current_user, contact = new_contact,
            description=oneonone_notes, form_endpoint = an_response[1],
            other_endpoint=an_response[2])
        new_oneonone.save()
        return redirect('/')
...
def edit_one_on_one(request, one_on_one_id):
  if request.method == 'POST':
    ...
    if(errors == []):
      ...
      #submit one on one data to action network using Tim's script
      an_response = an_put(current_oneonone.form_endpoint, current_oneonone.other_endpoint, current_user.first_name, current_user.last_name, current_user.email, contact_first_name, contact_last_name, contact_email, oneonone_notes)
      print(an_response)
      if(an_response[0] == 200):
        #checking if contact exists, if so we use the id of the contact already in the system
